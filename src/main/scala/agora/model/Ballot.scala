...
case class DmpBallot(preferences: Either[Candidate, List[Candidate]], override val id: Int, override val weight: Rational)
  extends Ballot(id, weight) {
  lazy val firstVotes = preferences.fold(
    candidate => {
      Map(candidate -> Rational(1, 1))
    },
    list => {
